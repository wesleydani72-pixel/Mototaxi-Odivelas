...
export function initDatabase() {
  ...
  if (!localStorage.getItem(STORAGE_KEYS.USERS)) {
    ...
    // Garantir que jl6568402@gmail.com e wesleydani72@gmail.com existem no localStorage
    try {
      ...
      if (currentUsersStr) {
        ...
        const adminWesleyExiste = currentUsers.some(u => u.role === 'admin' && u.email?.toLowerCase() === 'wesleydani72@gmail.com');
        if (!adminWesleyExiste) {
          const adminWesley: AdminUser = {
            id: 'admin_wesley',
            role: 'admin',
            nome: 'Wesley Pereira Ferreira',
            email: 'wesleydani72@gmail.com',
...
async function seedFirestoreIfNeeded() {
  ...
  try {
    ...
    if (snapshot.empty) {
      ...
      // Se não estiver vazio, garantir que ambos os administradores estão presentes no snapshot do Firestore
      let adminExiste = false;
      let adminWesleyExiste = false;
      snapshot.forEach((doc) => {
        const data = doc.data();
        if (data && data.role === 'admin') {
          if (data.email?.toLowerCase() === 'jl6568402@gmail.com') {
            adminExiste = true;
          }
          if (data.email?.toLowerCase() === 'wesleydani72@gmail.com') {
            adminWesleyExiste = true;
          }
        }
      });
      if (!adminExiste) {
      ...
      if (!adminWesleyExiste) {
        console.log("Administrador oficial wesleydani72@gmail.com não encontrado no Firestore. Criando automaticamente...");
        const adminWesley: AdminUser = {
          id: 'admin_wesley',
          role: 'admin',
          nome: 'Wesley Pereira Ferreira',
        ...
        await setDoc(doc(firestore, 'users', adminWesley.id), sanitizeForFirestore(adminWesley));
      }
    }
  } catch (error) {
