...
export function AdminDashboard() {
  ...
  // Estado para Toast / Mensagem de Sucesso
  const [adminToast, setAdminToast] = useState<{ show: boolean; message: string; type: 'success' | 'error' }>({ show: false, message: '', type: 'success' });

  const showToast = (message: string, type: 'success' | 'error' = 'success') => {
    setAdminToast({ show: true, message, type });
    setTimeout(() => {
  ...
  const baixarClientePDF = (c: ClienteUser) => {
    ...
    // 3. Foto de Perfil na primeira página
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(15, 23, 42);
    doc.text('2. FOTO DE PERFIL DO PASSAGEIRO', 15, currentY);
  ...
  const handleCadastrarTodosTurnos = () => {
    ...
    saveConfig(nextCfg);
    setConfigState(nextCfg);
    showToast('Todos os turnos (Manhã, Tarde, Noite, Madrugada) foram configurados com sucesso!', 'success');
  };
  ...
  return (
    <div className="flex-1 bg-slate-50 flex overflow-hidden font-sans text-slate-800">
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="w-full max-w-lg bg-white rounded-[36px] shadow-2xl p-8 space-y-6">
            ...
            <form onSubmit={handleSalvarTarifa} className="space-y-5">
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Nome do Turno</label>
                <input
                  type="text"
                  value={tTurno}
                  onChange={e => setTTurno(e.target.value)}
                  required
                  placeholder="Ex: Manhã, Tarde, Especial..."
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:bg-white"
                />
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in print:hidden">
          <div className="w-full max-w-3xl bg-white rounded-[36px] shadow-2xl p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              ...
              {/* ETAPA 2: FOTOS E DOCUMENTOS ENVIADOS */}
              <div className="bg-slate-50 rounded-3xl p-5 border border-slate-200/60 space-y-3">
                <h4 className="font-extrabold text-slate-900 text-sm tracking-tight flex items-center gap-1.5 border-b border-slate-200 pb-2 mb-2">
      ...
      {adminToast.show && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 text-white font-bold text-xs px-5 py-3.5 rounded-2xl shadow-xl flex items-center gap-2 border border-slate-800 animate-fade-in">
