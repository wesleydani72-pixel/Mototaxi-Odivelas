...
interface ChamadaPendenteCardProps {
  key?: string;
  ride: Ride;
  moto: MototaxistaUser;
  onAceitar: (ride: Ride, valorAdicional: string, tempoChegadaInput: string) => void;
  onRecusar: (ride: Ride) => void;
}

function ChamadaPendenteCard({ ride, moto, onAceitar, onRecusar }: ChamadaPendenteCardProps) {
  ...
  return (
    <div className="w-full md:max-w-md bg-slate-900 border-2 border-yellow-400 rounded-[36px] shadow-2xl p-6 sm:p-8 text-white space-y-6 relative overflow-y-auto max-h-[85vh] ring-12 ring-yellow-400/10 flex flex-col justify-between">
      <div className="absolute top-0 left-0 right-0 h-4 bg-yellow-400 animate-pulse rounded-t-3xl" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-800">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h4 className="text-yellow-400 text-[10px] sm:text-xs font-black uppercase tracking-wider">Nova Corrida Disponível!</h4>
              <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                <p className="text-xl sm:text-2xl font-black text-white truncate max-w-[180px] sm:max-w-none">{ride.clienteNome}</p>
                {ride.clienteFoto ? (
                  <img 
                    src={ride.clienteFoto} 
                  ...
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-slate-800 text-slate-400 flex items-center justify-center text-xs font-bold border border-slate-700 shrink-0 uppercase">
                    {ride.clienteNome ? ride.clienteNome.slice(0, 2) : 'CLI'}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-slate-800 border border-slate-700/80 px-4 py-2.5 rounded-2xl text-left sm:text-right shrink-0 sm:self-center">
            ...
            <p className="text-2xl sm:text-3xl font-black text-yellow-400 font-mono leading-none mt-1">
              R$ {(ride.valorEstimado + (ride.isZonaRural ? (parseFloat(valorAdicional) || 0) : 0)).toFixed(2)}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4 bg-slate-800/80 p-6 rounded-3xl border border-slate-700 text-xs">
          {ride.isEntrega && (
            <div className="p-4 bg-blue-950/50 border border-blue-500/30 rounded-2xl space-y-2">
              <span className="px-2 py-0.5 bg-blue-500 text-white font-black text-[9px] rounded uppercase block w-max tracking-wider">
                📦 SERVIÇO DE ENCOMENDA ({ride.entregaTipo === 'levar' ? 'LEVAR PACOTE' : 'BUSCAR ENCOMENDA'})
              </span>
              <p className="font-bold text-slate-200">📦 Item a Transportar: <span className="font-extrabold text-white">{ride.itemTransportado}</span></p>
              {ride.destinatarioNome && <p className="font-bold text-slate-200">👤 Recebedor: <span className="font-extrabold text-white">{ride.destinatarioNome}</span></p>}
              {ride.fotoEncomenda && (
                <div className="pt-1">
                  <p className="text-slate-400 text-[10px] uppercase font-bold mb-1">📸 Foto da Encomenda:</p>
                  <img src={ride.fotoEncomenda} alt="Foto da Encomenda" className="w-20 h-20 object-cover rounded-xl border border-slate-700 inline-block" />
                </div>
              )}
            </div>
          )}
          <div>
            <span className="font-bold text-slate-400 uppercase text-[10px]">📍 Ponto de Embarque:</span>
            <p className="text-sm font-extrabold text-white mt-1">
              {ride.origem ? ride.origem.replace(/\s*\(Região:[^)]*\)/gi, '').replace(/^Zona Rural\s*-\s*/gi, '') : ''}
            </p>
            {ride.refOrigem && !ride.refOrigem.toLowerCase().includes('identificado via gps') && (
              <p className="text-slate-300 mt-0.5">Ref: {ride.refOrigem}</p>
            )}
          </div>
            ...
            {ride.isZonaRural ? (
              <div className="mt-1 p-3.5 bg-yellow-450/25 border border-yellow-450/40 rounded-2xl">
                ...
                <p className="text-sm font-black text-yellow-450">
                  {ride.destino ? ride.destino.replace(/^Zona Rural\s*-\s*/gi, '') : ''}
                </p>
              </div>
            ) : (
              <p className="text-sm font-extrabold text-white mt-1">
                {ride.destino ? ride.destino.replace(/\s*\(Região:[^)]*\)/gi, '').replace(/^Zona Rural\s*-\s*/gi, '') : ''}
              </p>
            )}
          </div>
          ...
          {ride.fotoOrigem && (
            <div className="pt-2">
              ...
              <img src={ride.fotoOrigem} alt="Foto" className="w-full h-32 object-cover rounded-2xl border border-slate-600" />
            </div>
          )}
        </div>
      ...
      <div className="flex gap-4 pt-4 mt-4 border-t border-slate-800">
        <button
          onClick={() => onRecusar(ride)}
          className="flex-1 py-4 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-2xl font-bold text-xs transition-all cursor-pointer"
        >
          Recusar
        </button>
        <button
          onClick={() => onAceitar(ride, valorAdicional, tempoChegadaInput)}
          className="flex-[2] py-4 bg-yellow-400 hover:bg-yellow-500 text-slate-950 rounded-2xl font-black text-sm shadow-[0_0_20px_rgba(250,204,21,0.2)] transition-all cursor-pointer flex items-center justify-center gap-1.5"
        >
...
export function DriverDashboard() {
  ...
  const [chamadasPendentes, setChamadasPendentes] = useState<Ride[]>([]);
  ...
  // Listen to pending rides in real-time from Firestore (solves concurrency by supporting multiple concurrent requests)
  useEffect(() => {
    if (!firestore || !moto?.id || moto.disponibilidade !== 'disponivel' || corridaAtiva) {
      setChamadasPendentes([]);
      return;
    }

    const q = query(
      collection(firestore, 'rides'),
      where('status', 'in', ['aguardando', 'SOLICITADA', 'AGUARDANDO_MOTORISTA'])
    );
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        ...
        const semMotorista = !data.mototaxistaId;
        const naoRecusada = !(data.recusadoPor || []).includes(moto.id);

        if (semMotorista && naoRecusada) {
          list.push({ ...data, id: rideId });
        }
      });

      // Sort by creation time (most recent first)
      list.sort((a, b) => (b.criadoEm || 0) - (a.criadoEm || 0));
      setChamadasPendentes(list);
    }, (error) => {
      console.error('Erro ao escutar corridas do Firestore:', error);
    });

    return () => unsubscribe();
  }, [firestore, moto?.id, moto?.disponibilidade, !!corridaAtiva]);
        ...
        if (notif.tipo === 'nova_corrida' && notif.rideId) {
          ...
          if (found) {
            if ((found.status === 'aguardando' || found.status === 'SOLICITADA' || found.status === 'AGUARDANDO_MOTORISTA') && !(found.recusadoPor || []).includes(moto.id)) {
              setChamadasPendentes(prev => {
                if (prev.some(r => r.id === found.id)) return prev;
                return [found, ...prev];
              });
            }
          } else {
  ...
  const handleAceitarChamada = (ride: Ride, extraValStr: string, eta: string) => {
    ...
    if (!fresh || !['aguardando', 'SOLICITADA', 'AGUARDANDO_MOTORISTA'].includes(fresh.status) || fresh.mototaxistaId) {
      ...
      setChamadasPendentes(prev => prev.filter(r => r.id !== ride.id));
      return;
    }

    const extraVal = fresh.isZonaRural ? (parseFloat(extraValStr) || 0) : 0;
    const etaVal = eta || '10 min';
    ...
    addNotification({
      ...
      mensagem: `${moto.nome} (${moto.modeloMoto} - ${moto.placa}) aceitou sua corrida! Tempo estimado de chegada: ${etaVal}. Acompanhe no mapa em tempo real.`,
      tipo: 'corrida_aceita',
      rideId: aceita.id
    });
  ...
  return (
    <div className="flex-1 bg-slate-50 p-4 sm:p-8 overflow-y-auto font-sans text-slate-800 relative">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {/* ================================================================================= */}
        {/* OVERLAY REAL-TIME DE NOVAS CHAMADAS SIMULTÂNEAS (CONCORRÊNCIA RESOLVIDA) */}
        {/* ================================================================================= */}
        {chamadasPendentes.length > 0 && moto.disponibilidade === 'disponivel' && (
          <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex flex-col items-center justify-start p-4 md:p-8 overflow-y-auto animate-fade-in">
            {/* Header informando sobre chamadas pendentes */}
            <div className="w-full max-w-4xl text-center mb-6 pt-4 shrink-0">
              <span className="px-4 py-1.5 bg-yellow-400/20 text-yellow-400 font-black text-xs uppercase tracking-widest rounded-full border border-yellow-400/30 animate-pulse">
                🔔 {chamadasPendentes.length} {chamadasPendentes.length === 1 ? 'Solicitação de Corrida Ativa' : 'Solicitações de Corridas Ativas'}
              </span>
              <h2 className="text-2xl md:text-3xl font-black text-white mt-2">Escolha uma corrida para aceitar</h2>
              <p className="text-xs text-slate-400 mt-1">Você pode analisar todas as chamadas ativas e aceitar a melhor para você!</p>
            </div>

            {/* List of active cards */}
            <div className="w-full max-w-5xl flex flex-col md:flex-row flex-wrap gap-6 items-stretch justify-center pb-8">
              {chamadasPendentes.map((ride) => (
                <ChamadaPendenteCard 
                  key={ride.id} 
                  ride={ride} 
                  moto={moto}
                  onAceitar={handleAceitarChamada}
                  onRecusar={handleRecusarChamada}
                />
              ))}
            </div>
          </div>
